...
def obtain_points(lst):
    ...
    """
    objects = ["HIP {}".format(x) for x in lst]#Append identifier to number
    query = Simbad.query_objects(objects)#Search for objects in list
...
def download_points(lat, long, index_list, download_path):
    """
    ...
    file = open('{}/constellation_data.txt'.format(download_path), 'w+')
    latitude = [] #initialise an empty list
    longitude = [] #initialise an empty list
    start = 0 #initialise a variable
    #Procedure is same as in the plotting function
    for stop in index_list:
        latitude.append([x for x in lat[start:start+stop]])
        longitude.append([x for x in long[start:start+stop]])
        start += stop
    file.write('Longitude\n')
    file.writelines('{}'.format(longitude))
    file.write('\nLatitude\n')
    file.writelines('{}'.format(latitude))
    file.close()
...
path = '/home/sahyadri/Desktop/CAMS-India'
download_points(lat, long, list_indices, path)